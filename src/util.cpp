...
int nAnonymizeOPTIONAmount = 500;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OPTION
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OPTION
    // Mac: ~/Library/Application Support/OPTION
    // Unix: ~/.OPTION
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OPTION";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "option.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "optiond.pid"));
