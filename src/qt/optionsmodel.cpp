...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOPTIONAmount"))
        settings.setValue("nAnonymizeOPTIONAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeOPTIONAmount = settings.value("nAnonymizeOPTIONAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeOPTIONAmount"))
        SoftSetArg("-anonymizeoptionamount", settings.value("nAnonymizeOPTIONAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeOPTIONAmount:
            nAnonymizeOPTIONAmount = value.toInt();
            settings.setValue("nAnonymizeOPTIONAmount", nAnonymizeOPTIONAmount);
            emit anonymizeOPTIONAmountChanged(nAnonymizeOPTIONAmount);
            break;
        default:
            break;
        }
