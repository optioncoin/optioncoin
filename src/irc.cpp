...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #optioncoinTEST\r");
            Send(hSocket, "WHO #optioncoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #optioncoin\n");
            // randomly join #optioncoin00-#optioncoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #optioncoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #optioncoin%02d\r", channel_number).c_str());
        };
